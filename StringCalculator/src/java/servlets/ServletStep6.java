 ...
 */
public class ServletStep6 extends HttpServlet {
    ...
    private int add(String numbers){
        if(numbers.length() == 0){
            ...
            if(numbers.startsWith("//")){
                String firstRow = numbers.substring(0, numbers.indexOf("\n"));
                delimiter = firstRow.substring(2, firstRow.length());
            ...
            for(int i=0;i<nums.length;i++){
                Integer n = nums[i].equals("") ? 0 : Integer.valueOf(nums[i]);
                if(n<0){
                    negatives.add(n);
                }
                if(n > 1000){
